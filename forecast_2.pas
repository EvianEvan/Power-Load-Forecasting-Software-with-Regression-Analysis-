...
procedure TForm8.Button2Click(Sender: TObject);
  ...
  year_sum,load_sum,
  xy,xx,yy,cc,sd,
  input_year,      //w是使用TryStrTofloat定义的临时变量
  ...
  //判断输入的字符是否为数字
  if not TryStrTofloat(Trim(edit1.Text),w) then
  begin
    Application.MessageBox('您输入的GDP不符合要求，请重新输入。',
    '错误提示', MB_OK + MB_ICONSTOP);
    edit1.SetFocus ;
    exit;
  end;
  //根据输入的GDP进行负荷预测
  input_year := StrTofloat(EDIT1.TEXT);
